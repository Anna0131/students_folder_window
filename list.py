...
def open_folder():
    ...
    folder_path = r"C:\code_practice\json_student\json_student\01686463" 
...
def display_file(filename, nb, filepath): 
    ...
    if filename.endswith(".json") and "修課紀錄" in filename: # 如果文件名以 ".json" 結尾      
        ...
        nb.add(page, text="修課紀錄") # 將 ttk.Frame 添加到 ttk.Notebook 對象中           
        create_json_table(filename,tree,0)

        page0 = ttk.Frame(nb) # 創建新的 ttk.Frame 對象
        tree0 = ttk.Treeview(page0, columns=("學年度", "學期", "課程類別", "科目名稱", "學分數", "學業成績", "推估相對表現"))
        nb.add(page0, text="語文領域課程") # 將 ttk.Frame 添加到 ttk.Notebook 對象中           
        create_json_table(filename, 0,tree0)
...
def create_json_table(filename,tree,tree0):
    ...
    for column in tree["columns"]:
        ...
        tree.column(column, anchor="center")# 設置表格欄位名稱並置中對齊
    # 設置第 0 欄的寬度為 0
    ...
    # 取得修課紀錄資料(上)
    #record000 =data[list(data.keys())[1]] 
    ...
    for record in records:
        for dic in record["課程領域資訊"]:
            if record[dic] == 01:
                ...
                tree0.insert("", "end", values=row)
    # 設置Treeview的大小和位置
    tree0.pack(fill="both", expand=True)

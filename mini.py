...
cur_nb = [] # 目前 nb
def open_original_folder():# 走遍所選資料夾的學生
    #all_student_path=tk.filedialog.askdirectory() #使用者上傳資料夾路徑    
    all_student_path= r"C:\code_practice\json_student\json_student\000001"
    if all_student_path: # 如果選擇了資料夾(為真或非空)
        #  創一個 Notebook 物件 nb 放置於 root 主視窗中，可以將多個頁面加到 nb 中
        nb = ttk.Notebook(root) # Notebook 是 ttk 模組的一個 widget，可以在其中創建多個頁面分別顯示不同的內容        
        nb.pack(fill="both", expand=True) # 
        nb.pack(side="bottom", fill="both", expand=True) # nb 放在視窗下方並展開
         # 走遍所選資料夾下的所有檔案
        for each_student in os.listdir(all_student_path): # os.listdir()用於列出資料夾中的所有檔案名稱       
            # 完整的檔案路徑 = 上傳的資料夾路徑(folder_path)和遍歷到的檔案名稱(filename)組合在一起 
            student_path=os.path.join(all_student_path, each_student)   
            all_students(each_student,student_path, nb) # 將檔案顯示在 ttk.Notebook 中


def all_students(each_student,student_path, nb):   
    frame = tk.Frame(nb)
    frame.pack(fill="both", expand=True)
    button = tk.Button(frame, text=each_student, command=lambda: open_folder(student_path))
...
def open_folder(folder_path):
    #folder_path = tk.filedialog.askdirectory() #使用者上傳資料夾路徑       
    #folder_path = r"C:\Users\annnn\OneDrive\桌面\json_student\01686463" 
    if folder_path: # 如果選擇了資料夾(folder_path 為真或非空)
        deleteCurNb() # 刪除目前已經有的 nb
        nb = ttk.Notebook(root) # 可以在nb創建多個頁面分別顯示不同的內容       
        nb.pack(fill="both", expand=True) # 將 ttk.Notebook 對象填充整個窗口並展開

         # 走遍所選資料夾下的所有檔案       
        for filename in os.listdir(folder_path): # os.listdir列出資料夾中的所有檔案名稱   
            # filepath(完整的檔案路徑) = 上傳的資料夾路徑(folder_path)和遍歷到的檔案名稱(filename)組合在一起 
            filepath = os.path.join(folder_path, filename) 
            display_file(filepath, nb) # 將檔案顯示在 ttk.Notebook 中
        cur_nb.append(nb) # 新增目前的 nb

def deleteCurNb() : # 刪除目前已經有的 nb
    for each_nb in cur_nb :
        each_nb.destroy()
        cur_nb.remove(cur_nb[0])

# 顯示指定類型的文件內容
def display_file(filepath, nb): # filename = 要顯示的文件名/nb = 顯示 JSON 文件內容的 ttk.Notebook 對象 ，可以將多個頁面添加到nb中

    if filepath.endswith(".json") and "修課紀錄" in filepath: # 如果文件名以 ".json" 結尾      
        page = ttk.Frame(nb) # 創建新的 ttk.Frame 對象
        tree = ttk.Treeview(page, columns=("學年度", "學期", "課程類別", "科目名稱", "學分數", "學業成績", "推估相對表現"))
        nb.add(page, text=((os.path.basename(filepath)).split("_")[-1]).split(".json")[0]) # 將 ttk.Frame 添加到 ttk.Notebook 對象中       
        #main = setCanvas(page)  
           
        create_json_table(filepath,tree)
     

    elif filepath.endswith(".json") and "LIST" not in filepath: # 如果文件名以 ".json" 結尾      
        page = ttk.Frame(nb) # 創建新的 ttk.Frame 對象
        nb.add(page, text=((os.path.basename(filepath)).split("_")[-1]).split(".json")[0]) # 將 ttk.Frame 添加到 ttk.Notebook 對象中    
        main = setCanvas(page)  
        ...
        if "多元表現綜整心得" or "其他" or "學習歷程自述" in filepath:
            page = ttk.Frame(nb) # 創建新的 ttk.Frame 對象
            nb.add(page, text=((os.path.basename(filepath)).split("_")[-1]).split(".json")[0]) # 將 ttk.Frame 添加到 ttk.Notebook 對象中
            main = setCanvas(page)
        ...
        for key, value in data.items(): # 字典每一項鍵值對         
            ...
            if isinstance(value, dict): # 如果value是字典
                ...
                for dictionary in value: #該list中的各個dictionary 
                    ...
                    ttk.Label(page, text="---------------------------------------------").grid(column=0, row=row, sticky="W")
                    row+=1
...
#root.geometry("800x600+{}+{}".format(int((root.winfo_screenwidth() - 800) / 2), int((root.winfo_screenheight() - 600) / 2)))


open_original_folder()#直接讓使用者上傳資料夾

...
def create_json_table(filename, tree):
    ...
    # 所以課程領域("語文領域","數學領域","社會領域","自然科學領域","藝術領域","綜合活動領域","科技領域","健康與體育領域","全民國防教育")
    domains = [
        "語文領域",
        "數學領域",
        "社會領域",
    ...
    domain_filter_frame.pack(side=tk.BOTTOM, fill=tk.Y)  # 設置該 Frame 在 tree中的位置
    ...
    domain_filter.pack(anchor="w")  

    # 當使用者選擇課程領域時觸發
    def on_domain_select(event):
        selected_value = domain_filter.get()
        filter_courses_by_domain(records, tree, domain_filter, selected_value) 
    # 獲取選擇的課程領域，根據該領域篩選符合條件的課程資料，更新顯示在表格 tree 上的資料
    domain_filter.bind("<<ComboboxSelected>>", on_domain_select)
    ...
    def filter_courses_by_domain(records, tree, domain_filter, selected_domain):
        # 獲取所有的課程領域（不再需要重新計算）
        course_domains = set([get_domain(record) for record in records]) # set確保每個課程領域只出現一次

        # 將課程領域的項目設置為下拉選單的選項
        domain_filter["values"] = list(course_domains)
